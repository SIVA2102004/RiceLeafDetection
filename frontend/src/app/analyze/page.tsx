          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
              <div className="p-3.5 rounded-xl border bg-emerald-50 border-emerald-300">
                <p className="text-xs text-emerald-800 font-medium">{t.estimatedSeverity}</p>
              ...
              <div className="bg-white p-3.5 rounded-xl border border-emerald-200">
                <p className="text-xs text-emerald-800 font-medium">{t.affectedArea}</p>
                <p className="font-bold text-emerald-900 text-base mt-0.5">
                  {result.affected_area_percentage}%
                </p>
              </div>
              <div className="bg-white p-3.5 rounded-xl border border-emerald-200">
                <p className="text-xs text-emerald-800 font-medium">{t.modelPipeline}</p>
                <p className="font-semibold text-emerald-900 text-xs mt-1 truncate">
            ...
            <div className="rounded-xl p-4 border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs bg-emerald-50/90 border-emerald-300 text-emerald-950">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded text-[11px] font-bold uppercase bg-emerald-800 text-white">
                    {result.severity === "High" ? "Critical / Action Required" : result.severity === "Moderate" ? "Spreading / Active" : "Mild / Early Stage"}
                  </span>
                  <span className="font-bold text-emerald-950">What It Means for the Farmer:</span>
                </div>
              ...
              <div className="sm:min-w-[240px] p-2.5 rounded-lg border text-xs bg-white border-emerald-400 text-emerald-950 shadow-xs">
                <strong className="block font-bold text-[11px] uppercase tracking-wide mb-0.5 text-emerald-900">Immediate Action Needed:</strong>
                <p className="text-[11px] leading-tight text-emerald-850">
                  {result.severity === "High"
            ...
            {result.expert_review_required && (
              <div className="bg-white border-2 border-emerald-600 p-3.5 rounded-xl flex items-start gap-2.5 text-emerald-950 text-xs shadow-xs">
                <ShieldAlert className="w-4 h-4 text-emerald-700 flex-shrink-0 mt-0.5" />
                <p>
                  <strong className="text-emerald-900">{t.needsAgronomicReview}</strong> {t.needsAgronomicReviewText}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200 space-y-3">
              ...
              <ul className="space-y-2 text-xs text-stone-700">
                {displayManagement.map((m, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-800 mt-1.5 flex-shrink-0" />
                    <span>{m}</span>
                  </li>
                ))}
              </ul>
            ...
            {feedbackSent ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 bg-white hover:bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-lg border border-emerald-300 transition font-medium"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1 bg-white hover:bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-lg border border-emerald-300 transition font-medium"
                >
                  <ThumbsDown className="w-3.5 h-3.5 text-emerald-700" />
                  <span>{t.no}</span>
                </button>
              </div>
            )}
